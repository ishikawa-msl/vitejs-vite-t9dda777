...
export default function App() {
  ...
  // あなたのGAS URL
  const SPREADSHEET_GAS_URL =
    'https://script.google.com/a/macros/mitsui-soko-lg.com/s/AKfycbxqABK9uxF_v12bwzQ5xo13MaYQzly_5C35y6TclY5M3P4p_9oH33QWreXmY3W4/exec';

  // スキャンライブラリ & Tailwind CSS (デザイン) の強制読み込み
  useEffect(() => {
    // スキャンライブラリの読み込み
    ...
    // デザイン(Tailwind)が当たらない場合のための強制読み込み
  ...
  const handleStartScan = (field: string) => {
    ...
    setTimeout(() => {
      ...
      if (typeof Html5Qrcode === 'undefined') {
        setCameraError(
          'ライブラリを読み込み中です。数秒待ってからやり直してください。'
        );
        return;
      }
      try {
  ...
  const handleSave = async () => {
    if (!SPREADSHEET_GAS_URL)
      return alert(
        'URLが設定されていません。コード内のURLを確認してください。'
      );
    setIsSaving(true);
    ...
    try {
      ...
      alert(
        'エラーが発生しました。GASのデプロイ設定を「全員」にしているか確認してください。'
      );
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-40 font-sans overflow-x-hidden">
      ...
      <main className="max-w-md mx-auto p-5 space-y-8 animate-in fade-in duration-500">
        ...
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
            <Icon name="scan" size={14} className="text-blue-500" />{' '}
            スキャン・数量確認
          </h2>
          <div className="space-y-3">
            {['JANコード', '型番'].map((f) => (
              <div
                key={f}
                className="bg-white rounded-2xl border border-slate-200 p-4 flex justify-between items-center shadow-sm"
              >
                <div className="flex-1 mr-4">
                  ...
                  <input
                    type="text"
                    value={f === 'JANコード' ? formData.jan : formData.model}
                    onChange={(e) =>
                      updateField(
                        f === 'JANコード' ? 'jan' : 'model',
                        e.target.value
                ...
                <button
                  onClick={() => handleStartScan(f)}
                  className="bg-blue-600 text-white px-3 py-2 rounded-xl text-[10px] font-bold shadow-md active:scale-95 transition-all"
                >
                  <Icon name="scan" size={14} className="inline mr-1" />{' '}
                  スキャン
                </button>
              </div>
            ))}
            ...
            <div className="bg-blue-50/50 rounded-2xl border border-blue-200 p-4 flex justify-between items-center text-blue-700 shadow-sm">
              ...
              <input
                ...
                className="w-20 bg-white border border-blue-300 rounded-lg text-center font-bold py-2 text-lg outline-none"
                placeholder="0"
              />
            </div>
          </div>
        ...
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
            <Icon name="alert" size={14} className="text-red-500" /> 不備内容
          </h2>
          <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm space-y-4">
            <div className="grid grid-cols-2 gap-2">
              {['数量違い', '破損', '納品書なし', 'その他'].map((l) => (
                <button
                  key={l}
                  onClick={() => toggleIssue(l)}
                  className={`p-3 rounded-2xl border text-[11px] font-bold transition-all flex items-center justify-center gap-2 ${
                    formData.issues.includes(l)
            ...
            {formData.issues.includes('その他') && (
              <textarea
                ...
                className="w-full mt-1.5 bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm h-32 outline-none focus:bg-white resize-none"
                placeholder="不備の詳細を入力してください"
              />
            )}
          </div>
        </section>
        ...
        <div className="fixed bottom-10 left-0 right-0 p-5 bg-gradient-to-t from-slate-50 flex justify-center z-20 pointer-events-none">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className={`bg-blue-600 max-w-md w-full py-5 rounded-2xl font-bold text-white shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3 pointer-events-auto ${
              isSaving ? 'opacity-50' : ''
            }`}
          >
            {isSaving ? (
              ...
              <>
                <Icon name="save" size={22} /> 保存して報告を送信
              </>
            )}
          </button>
        </div>
      ...
      {isScanning && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center p-4 animate-in fade-in duration-300">
          ...
          <div
            id="reader"
            className="w-full max-w-sm overflow-hidden rounded-2xl bg-zinc-900 border-2 border-white/20 shadow-2xl aspect-square"
          ></div>
          {cameraError && (
            <p className="text-red-400 text-xs mt-6 px-4 text-center font-bold">
              {cameraError}
            </p>
          )}
          <p className="text-white mt-10 text-sm font-bold tracking-widest">
            {scanField}をスキャン中...
          </p>
        </div>
      )}
